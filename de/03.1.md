...
Jedes mal wenn Du deinen Browser öffnest, eine URL eingibst und Enter drückst, erschein eine mehr oder weniger hübsche Webseite auf deinem Bildschirm. Aber weißt du auch was hinter dieser einfachen Aktion passiert?

Im normalfall ist dein Browser ein client. Nachdem du eine URL eingegeben hast, nimmt er den Host der URL und sendet diesen an einen DNS server um die IP Adresse des Hosts zu erfragen. Dann kontaktiert er die IP Adresse und fragt nach einer TCP verbindung. Der Browser sendet HTTP anfragen an diese Verbindung. Der Server verarbeitet diese Anfragen und sendet HTTP antworten (engl. responses) die die Informationen zum erzeugen der Webseite enthalten. Zum Schluss rendert der Browser den sichtbaren Teil (den body) der Webseite und beendet die Verbindung zum Server.

![](images/3.1.web2.png?raw=true)

Abbildung 3.1 Ablauf eines Webseitenaufrufs

Ein Webserver, auch HTTP server genannt, verwendet das HTTP Protokoll um mit den Clients zu kommunizieren. Alle Webbrowser werden als Clients angesehen. 

Wir können die Arbeitsweise des Webs also in folgende Schritte aufteilen: 

- Client nutzen das TCP/IP Protokoll zum verbinden mit Servern.
- Client senden HTTP Anfragenpackete (engl. request packages) zum Server.
- Server senden HTTP Antwortpakete zum Client. Wenn die Anfrage dynamische Inhalte anfordert, werden diese Inhalte zuerst erzeugt.
- Client schließt die Verbindung zum Server und rendert das HTML.

Dies ist ein einfacher Arbeitsablauf von HTTP-Angelegenheiten - Merke Dir das der Server die Verbindung schließt nachdem er Daten an den Client gesendet hat, danach wartet er auf neue Anfragen.

## URL und DNS Auflösung

Wir alle nutzen URLs um Webseiten aufzurufen, aber weißt Du auch wie URLs funktionieren?
